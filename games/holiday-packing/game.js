...
function resetRound() {
  ...
  updateHud();
    ...
    setCollectionMessage(
      "Pack the essentials — but leave the unnecesary items behind!"
    );
  } else if (round.id === 1) {
    setCollectionMessage(
...
if (!won) {
    ...
    ui.title.textContent =
      ...
      ui.text.textContent =
        `Louise and Steve scored ${displayedScore} points. ` +
        "Try again and get the suitcase packed before the holiday countdown ends!";
    } else if (isPhotoPuzzleRound()) {
      ui.text.textContent =
...
if (isPackingRound()) {
  ...
  giveUpButton.style.display =
    ...
    ui.title.textContent =
      "Holiday Packing Complete!";

    const packingTimedOut =
...
ui.text.textContent =
  packingTimedOut
    ? "The suitcase is packed! You got there in the end — 🥥 Coconut collected!"
    : "The suitcase is packed and Louise and Steve are ready for their holiday! 🍹 Cocktail earned!";
...
const packingGiveUp =
  document.getElementById("packingGiveUp");

if (packingGiveUp) {
  packingGiveUp.style.display = "none";
}
    resetRound();
    startRound();
  }
